...
func (s *FileServerService) startTrackingEX(podName string, nb *entity.FileServer) (*podtracker.PodTracker, error) {
	...
	podTracker.Track(func(pod *v1.Pod) bool {
		...
		logger.Infof("Tracking fileserver pod=%s phase=%s", podName, phase)

		switch phase {
		case "Pending":
			//s.SyncFromPod(nb, pod)
...
func (s *FileServerService) startTracking(podName string, nb *entity.FileServer) (*podtracker.PodTracker, error) {
	clientset, err := s.getClientset()
	if err != nil {
		return nil, err
	}
