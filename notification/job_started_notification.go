...
type StartedJobNotification struct {
	Job       *entity.Job
	Send      *entity.User
	CreatedAt time.Time
}

func NewStartedJobNotification(session *mongo.Session, job *entity.Job) (*StartedJobNotification, error) {
	uid := job.CreatedBy
...
func (sn *StartedJobNotification) RenderContent() (string, error) {
	// Define a template.
	const letter = `
Dear {{.Name}},
 
Your job {{.JobId}} {{.Link}} has started. 
...
func (sn *StartedJobNotification) RenderTitle() (string, error) {
	const subjectTml = `[aiForge] Job {{.JobId}} has started`
	type Content struct {
		JobId string
	}
